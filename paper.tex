...
To validate real-world applicability, three specific test cases were simulated (Table~\ref{tab:scenarios}).

\begin{table}[H]
\caption{Scenario Testing Results}
\begin{center}
\begin{tabular}{llcc}
\toprule
\textbf{Scenario} & \textbf{Context} & \textbf{Aquifer} & \textbf{Prediction} \\
\midrule
Case 1 & Secure Household & Moderate & Low Risk \\
Case 2 & Vulnerable Household & Moderate & Moderate Risk \\
Case 3 & Vulnerable Household & Critical & High Risk \\
\bottomrule
\end{tabular}
\label{tab:scenarios}
\end{center}
...
\begin{itemize}
\item \textbf{Case 1}: A household with multiple water sources (piped + well) remains secure even in a moderately stressed aquifer
\item \textbf{Case 2}: A household in the same aquifer location but without reliable infrastructure faces increased risk
\item \textbf{Case 3}: A vulnerable household in a Deep/High Stress aquifer zone (Depth $>$ 75 m, Depletion $>$ 1.8 m/yr) is correctly classified as High Risk
\end{itemize}

These results demonstrate that the model appropriately integrates both hydrogeological conditions and household infrastructure factors.
